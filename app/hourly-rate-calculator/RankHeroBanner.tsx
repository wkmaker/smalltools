...
function getNumbeoInsights(rate: number, countryMatches: CountryMatch[], isEn = false): NumbeoInsights {
  ...
  const evaluated: EvaluatedCountry[] = numbeoCostOfLivingData.countries.map((c) => {
    ...
    const costDiffText = isEn
      ? (costDiffPercent > 0
          ? `${costDiffPercent}% cheaper than TW`
          : costDiffPercent < 0
            ? `${Math.abs(costDiffPercent)}% higher than TW`
            : 'Same cost as TW')
      : (costDiffPercent > 0
    ...
    if (costDiffPercent >= 40) {
      lifestylePerk = isEn ? 'Pool condo apartment + Dining & SPA freedom' : '泳池景觀公寓住到爽 + 外食SPA按摩自由';
    } else if (costDiffPercent >= 20) {
      lifestylePerk = isEn ? 'Modern elevator suite + Affordable dining' : '質感電梯大樓套房 + 外食手搖加蛋無負擔';
    } else if (costDiffPercent >= -10) {
      lifestylePerk = isEn ? 'Izakaya Yakiniku freedom + Urban culture experience' : '居酒屋燒肉自由 + 暢享高品質都會人文體驗';
    } else if (costDiffPercent >= -40) {
      lifestylePerk = isEn ? 'Exotic European lifestyle, budget planning needed' : '異國歐式風情體驗，外食租金開銷需稍微精算';
    } else {
      lifestylePerk = isEn ? 'Global top high-income tier, suits wealth allocation' : '全球頂尖高薪富豪圈，適合強大資產配置與精英身價';
    }
    ...
    return {
      ...
      costDiffText,
      localEffectiveRate,
      suitabilityTag: isEn && matchedSuitability.tag_en ? matchedSuitability.tag_en : matchedSuitability.tag,
      suitabilityName: isEn && matchedSuitability.name_en ? matchedSuitability.name_en : matchedSuitability.name,
      suitabilityDesc: isEn && matchedSuitability.description_en ? matchedSuitability.description_en : matchedSuitability.description,
      lifestylePerk,
    };
  });
  ...
  const currentTierName = isEn && currentTier.name_en ? currentTier.name_en : currentTier.name;
  const currentTierTag = isEn && currentTier.tag_en ? currentTier.tag_en : currentTier.tag;
  const tierTitle = isEn
    ? `【${currentTierTag}】Suits $${rate}/hr in ${currentTierName}`
    : `【${currentTier.tag}】適合時薪 $${rate}/hr 的${currentTier.name}`;
  const tierDesc = isEn && currentTier.description_en ? currentTier.description_en : currentTier.description;
...
export default function RankHeroBanner({
  ...
  const travelTiers = useMemo(
    () => getTravelTiers(displayHourlyRate, lang),
    [displayHourlyRate, lang]
  );
  ...
  const numbeoInsights = useMemo(
    () => getNumbeoInsights(displayHourlyRate, countryMatches, isEn),
    [displayHourlyRate, countryMatches, isEn]
  );
  ...
  if (displayHourlyRate <= 0) {
    statusBadge = isEn ? '0 TWD Free Tier! Universal Truth' : '0 元躺平！全球 152 國共通真理';
    statusBadgeStyle = 'bg-surface-glass border border-border-glass font-bold text-text-main';
    feelComment = isEn
      ? `At $0/hr in ${exploreTarget.country}, regardless of local dining or rent prices, 0 multiplied by any factor remains 0! Consider finding a job!`
      : `以你目前的 $0/hr 時薪移居${targetName}，不管當地外食多便宜或房租多划算，因為基礎薪資是 0，乘以任何倍數都還是 0 元！強烈建議先去夢裡躺平或快去尋找工作！`;
    comparisonVerdict = isEn
      ? `[0 TWD Evaluation] Challenging anywhere! Staying home with free AC is ideal!`
      : `【0 元體驗評估】去哪裡都是極致挑戰！不如待在家裡吹免費冷氣，夢裡要啥有啥！`;
  } else if (mult >= 1.25) {
    statusBadge = isEn ? 'Purchasing Power Upgrade!' : '降維打擊！生活品質爽度倍增';
    statusBadgeStyle = 'bg-surface-glass border border-border-glass font-bold text-text-main';
    feelComment = isEn
      ? `At $${displayHourlyRate}/hr in ${exploreTarget.country}, your purchasing power expands to ${exploreTarget.multiplierStr}x! Equivalent to $${localRate.toLocaleString('en-US')} NTD/hr locally.`
      : `以你目前 $${displayHourlyRate}/hr 的時薪移居${targetName}，購買力直接放大至 ${exploreTarget.multiplierStr}x 倍！相當於在當地享有 $${localRate.toLocaleString('zh-TW')} NTD/hr 的高可支配薪資。入住無邊際泳池公寓、外食美饌與 SPA 按摩完全不傷錢包！`;
    comparisonVerdict = isEn
      ? `[Power Upgrade] Living cost is ${costDiff}% lower than Taiwan, enabling rapid wealth accumulation.`
      : `【降維打擊】生活體驗評估：比在台灣生活輕鬆 ${costDiff}%，可用更低的生活負擔快速累積資產兼享度假生活！`;
  } else if (mult >= 0.85) {
    statusBadge = isEn ? 'Seamless Transition! Parallel Quality' : '無縫切換！與台灣生活品質對等';
    statusBadgeStyle = 'bg-surface-glass border border-border-glass font-bold text-text-main';
    feelComment = isEn
      ? `At $${displayHourlyRate}/hr in ${exploreTarget.country}, purchasing power matches 1:1 with Taiwan. Enjoy local culture smoothly with equal effort.`
      : `以你目前 $${displayHourlyRate}/hr 的時薪移居${targetName}，購買力與台灣維持 1:1 高度相當！日常外食與居住開銷平穩，能用同樣的努力輕鬆體驗異國文化與高品質都會生活步調。`;
    comparisonVerdict = isEn
      ? `[Seamless Swap] Living expenses are close to Taiwan. Enjoy local quality without financial friction!`
      : `【無痛切換】生活體驗評估：開銷與台灣極為接近，能無痛切換生活圈，暢享當地文化與頂級環境品質！`;
  } else if (mult >= 0.60) {
    statusBadge = isEn ? 'Great Environment, Budget Planning Needed' : '環境極佳，外食租金需精算';
    statusBadgeStyle = 'bg-surface-glass border border-border-glass font-bold text-text-main';
    const targetComfortableRate = Math.round(displayHourlyRate / mult);
    feelComment = isEn
      ? `At $${displayHourlyRate}/hr in ${exploreTarget.country}, enjoy high safety and cultural standards, though dining and rent are ${Math.abs(costDiff)}% higher than Taiwan.`
      : `以你目前 $${displayHourlyRate}/hr 的時薪移居${targetName}，能享受極高的治安與人文環境，但外食與租金比台灣高出 ${Math.abs(costDiff)}%，日常需適度分配預算。`;
    comparisonVerdict = isEn
      ? `[Target Guidance] To live comfortably in ${exploreTarget.country}, target hourly rate: $${targetComfortableRate.toLocaleString('en-US')} NTD/hr!`
      : `【建議試算】爽過建議：在${targetName}要過得十分寬裕，建議目標時薪拉升至 $${targetComfortableRate.toLocaleString('zh-TW')} NTD/hr！`;
  } else {
    statusBadge = isEn ? 'High Cost Metropolitan Area' : '高物價大都市，考驗預算能力';
    statusBadgeStyle = 'bg-surface-glass border border-border-glass font-bold text-text-main';
    const targetComfortableRate = Math.round(displayHourlyRate / mult);
    feelComment = isEn
      ? `At $${displayHourlyRate}/hr in ${exploreTarget.country}, expenses are ${Math.abs(costDiff)}% higher than Taiwan. Budgeting is required.`
      : `以你目前 $${displayHourlyRate}/hr 的時薪移居${targetName}，外食與房租開銷比台灣高出 ${Math.abs(costDiff)}%！外食大餐與精華區房租壓力較大，需精打細算。`;
    comparisonVerdict = isEn
      ? `[Goal Guidance] Local expenses are high. Recommended target hourly rate: $${targetComfortableRate.toLocaleString('en-US')} NTD/hr (~${(1 / mult).toFixed(1)}x current)!`
      : `【目標指引】爽過建議：當地開銷高昂，若要在${targetName}無憂無慮爽過，目標時薪建議拉升至 $${targetComfortableRate.toLocaleString('zh-TW')} NTD/hr (約當前的 ${(1 / mult).toFixed(1)} 倍)！`;
  }

  // ─── 分享處理器 ───────────────────────────────────────────────────────────
  const handleCopySocialText = async () => {
    const prCode = formatPrCode(heroMilestone.pr);
    const shareUrl = `${window.location.origin}/hourly-rate-calculator/${isEn ? 'en/' : ''}rank/${prCode}/?${queryParamsString}`;
    const text = isEn
      ? `\u3010Taiwan Worker PR Rating Card \ud83d\udcb3\u3011\nRating Level: PR ${displayPR}\u3010${heroMilestone.label_en || heroMilestone.label}\u3011 ${stars}\n\ud83d\udcac Trait Quote: "${heroMilestone.desc_en || heroMilestone.desc}"\n\u26a1 Real Hourly Earnings: $${displayHourlyRate}/hr (Value: $${displayMinuteValue}/min)\n\ud83e\udd64 Boba Freedom: 1 hr work = ${bobaCount} cups\n\ud83c\udfc6 Taiwan Rank: Approx. #${displayTaiwanRank} out of 8.4M workers!\n\ud83c\udf0d Migration Match: ${heroCountry.flag} ${heroCountry.name}\n\nCheck your true hourly rate \u2794 ${shareUrl}`
    ...
    <div className={`${styles.rankHeroBanner} mb-12 space-y-6`}>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border-glass pb-4">
        <div className="flex flex-wrap items-center gap-2">
          <span className={`px-3 py-1 text-xs font-bold rounded-full ${styles.milestoneBadge} flex items-center gap-1`}>
            ...
            <span>{displayPR === 0 ? (isEn ? 'Hidden Easter Egg PR 0.0' : '隱藏彩蛋解鎖 PR 0.0') : (isEn ? `Taiwan Salary Rank PR ${displayPR}` : `全台薪資評定 PR ${displayPR}`)}</span>
          </span>
          ...
          <h1 className="text-2xl sm:text-3xl font-black text-text-main tracking-tight">
            【{heroMilestone.label_en && isEn ? heroMilestone.label_en : heroMilestone.label}】
          </h1>
        </div>
      </div>
      ...
      <div className="p-4 rounded-xl bg-surface-glass border border-border-glass space-y-3">
        <div>
          <span className={`text-xs font-bold ${styles.themeAccentText} flex items-center gap-1 mb-1`}>
            ...
            {isEn ? 'Persona Trait Quote' : '階級特質語錄'}
          </span>
          <p className="text-base text-text-main font-medium leading-relaxed italic">
            「{heroMilestone.desc_en && isEn ? heroMilestone.desc_en : heroMilestone.desc}」
          </p>
        </div>
      </div>
      ...
      <div>
        <h3 className="text-xs font-bold text-text-sub mb-3 flex items-center gap-1.5 uppercase tracking-wider">
          ...
          {isEn ? 'Financial & Minute Value Metrics' : '核心財務與生命時間價值 (Financial & Minute Value)'}
        </h3>
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3">
          <div className="p-3 rounded-xl bg-surface-glass border border-border-glass">
            <span className="text-xs text-text-sub block mb-0.5">{isEn ? 'Est. Net Annual Income' : '推估實質年收益'}</span>
            <span className="text-sm font-extrabold text-text-main font-mono">${displayAnnualIncome.toLocaleString(isEn ? 'en-US' : 'zh-TW')}</span>
          </div>
          <div className="p-3 rounded-xl bg-surface-glass border border-border-glass">
            <span className="text-xs text-text-sub block mb-0.5">{isEn ? 'Est. Net Monthly Income' : '折算實質月收益'}</span>
            <span className="text-sm font-extrabold text-text-main font-mono">${displayMonthlyIncome.toLocaleString(isEn ? 'en-US' : 'zh-TW')}</span>
          </div>
          <div className="p-3 rounded-xl bg-surface-glass border border-border-glass">
            <span className="text-xs text-text-sub block mb-0.5">{isEn ? 'Est. Real Hourly Rate' : '預估實質時薪'}</span>
            <span className={`text-sm font-black font-mono ${styles.themeAccentText}`}>${displayHourlyRate} / hr</span>
          </div>
          <div className="p-3 rounded-xl bg-surface-glass border border-border-glass">
            <span className="text-xs text-text-sub block mb-0.5">{isEn ? 'Value per Minute' : '每分鐘生命價值'}</span>
            <span className="text-sm font-extrabold text-text-main font-mono">${displayMinuteValue} / {isEn ? 'min' : '分'}</span>
          </div>
          <div className="p-3 rounded-xl bg-surface-glass border border-border-glass">
            <span className="text-xs text-text-sub block mb-0.5">{isEn ? 'Minimum Wage Ratio' : '法定最低工薪倍數'}</span>
            <span className="text-sm font-extrabold text-text-main font-mono">{displayWageMultiplier}x {isEn ? '' : '倍'}</span>
          </div>
          <div className="p-3 rounded-xl bg-surface-glass border border-border-glass">
            <span className="text-xs text-text-sub block mb-0.5">{isEn ? 'Standard Monthly Hours' : '常態月工時'}</span>
            <span className="text-sm font-extrabold text-text-main font-mono">176 {isEn ? 'hrs' : '小時'}</span>
          </div>
        </div>
      </div>
      ...
      <div>
        <h3 className="text-xs font-bold text-text-sub mb-3 flex items-center gap-1.5 uppercase tracking-wider">
          ...
          {isEn ? 'Taiwan 8.4M Workers & Global Population Rank' : '全台 840 萬打工人與全球人口落點 (Ranking & Population)'}
        </h3>
          ...
          <div className="p-3.5 rounded-xl bg-surface-glass border border-border-glass">
            <div className="flex items-center justify-between mb-1">
              <span className="text-xs text-text-sub inline-flex items-center gap-1">
                ...
                {isEn ? 'Taiwan Worker Win Rate' : '全台打工人勝出率'}
              </span>
              <span className={`text-xs font-bold font-mono ${styles.themeAccentText}`}>PR {displayPR}</span>
            </div>
            <span className="text-sm font-bold text-text-main">
              {isEn ? (
                <>Ranked <span className={`text-base font-black font-mono ${styles.themeAccentText}`}>#{displayTaiwanRank}</span> out of 8.4M workers</>
              ) : (
          ...
          <div className="p-3.5 rounded-xl bg-surface-glass border border-border-glass">
            <div className="flex items-center justify-between mb-1">
              <span className="text-xs text-text-sub inline-flex items-center gap-1">
                ...
                {isEn ? 'Global Population Win Rate' : '全球人口勝出率'}
              </span>
              <span className={`text-xs font-bold font-mono ${styles.globalPrText}`}>PR {displayGlobalPR}</span>
            </div>
            <span className="text-sm font-bold text-text-main">
              {isEn ? (
                <>Outperformed <span className={`text-base font-black font-mono ${styles.globalPrText}`}>{displayGlobalBeatenPeople}B</span> people</>
              ) : (
          ...
          <div className="p-3.5 rounded-xl bg-surface-glass border border-border-glass">
            <div className="flex items-center justify-between mb-1">
              <span className="text-xs text-text-sub inline-flex items-center gap-1">
                ...
                {isEn ? 'DGBAS Official Tier' : '主計總處分位落點'}
              </span>
              <span className="text-xs font-bold font-mono text-text-sub">D1~D9</span>
            </div>
            <span className="text-sm font-bold text-text-main">
              {isEn ? `Official Tier: 【PR ${heroMilestone.pr} Tier】` : `官方薪資層級：【PR ${heroMilestone.pr} 分位階層】`}
            </span>
          </div>
        </div>
      </div>
      ...
      <div>
        <h3 className="text-xs font-bold text-text-sub mb-3 flex items-center gap-1.5 uppercase tracking-wider">
          ...
          {isEn ? 'Hourly Purchasing Power Index' : '趣味時薪購買力自由度指數 (Hourly Purchasing Power)'}
        </h3>
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
          <div className="p-3 rounded-xl bg-surface-glass border border-border-glass text-center">
            ...
            <span className="text-xs text-text-sub block mb-0.5">{isEn ? 'Boba Milk Tea' : '珍珠奶茶自由'}</span>
            <span className="text-xs font-bold text-text-main font-mono">{isEn ? `1 hr = ${bobaCount} cups` : `1小時 = ${bobaCount} 杯`}</span>
          </div>
          <div className="p-3 rounded-xl bg-surface-glass border border-border-glass text-center">
            <svg className={`w-5 h-5 mb-1 mx-auto ${styles.themeAccentText}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 8v13m0-13V3m0 0l-4 4m4-4l4 4M4 11h16M4 15h16M4 19h16" />
            </svg>
            <span className="text-xs text-text-sub block mb-0.5">{isEn ? 'Bento Lunchbox' : '排骨便當自由'}</span>
            <span className="text-xs font-bold text-text-main font-mono">{isEn ? `1 hr = ${bentoCount} meals` : `1小時 = ${bentoCount} 個`}</span>
          </div>
          <div className="p-3 rounded-xl bg-surface-glass border border-border-glass text-center">
            <svg className={`w-5 h-5 mb-1 mx-auto ${styles.themeAccentText}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <span className="text-xs text-text-sub block mb-0.5">{isEn ? 'Starbucks Latte' : '星巴克拿鐵自由'}</span>
            <span className="text-xs font-bold text-text-main font-mono">{isEn ? `1 hr = ${latteCount} cups` : `1小時 = ${latteCount} 杯`}</span>
          </div>
          <div className="p-3 rounded-xl bg-surface-glass border border-border-glass text-center">
            <svg className={`w-5 h-5 mb-1 mx-auto ${styles.themeAccentText}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 5v2m0 4v2m0 4v2M5 5a2 2 0 00-2 2v3a2 2 0 110 4v3a2 2 0 002 2h14a2 2 0 002-2v-3a2 2 0 110-4V7a2 2 0 00-2-2H5z" />
            </svg>
            <span className="text-xs text-text-sub block mb-0.5">{isEn ? 'Movie Ticket' : '威秀電影票自由'}</span>
            <span className="text-xs font-bold text-text-main font-mono">{isEn ? `Work ${movieHours} hrs/ticket` : `幹活 ${movieHours} 小時換1張`}</span>
          </div>
          <div className="p-3 rounded-xl bg-surface-glass border border-border-glass text-center">
            <svg className={`w-5 h-5 mb-1 mx-auto ${styles.themeAccentText}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <span className="text-xs font-bold text-text-main font-mono">{isEn ? `Work ${iphoneHours} hrs` : `需幹活 ${iphoneHours} 小時`}</span>
          </div>
        </div>
      </div>
      ...
      <div>
        <h3 className="text-xs font-bold text-text-sub mb-3 flex items-center gap-1.5 uppercase tracking-wider">
          ...
          {isEn ? 'Global Lifestyle Compatibility' : '全球生活圈與移居品質適配 (Global Lifestyle Compatibility)'}
        </h3>
          ...
          <div className="space-y-3">
            <div className="flex flex-wrap items-center justify-between gap-2 border-b border-border-glass pb-3">
              <div className="text-xs font-bold text-text-main flex items-center gap-1.5">
                ...
                <span>{isEn ? 'Travel Destinations (By Difficulty)' : '旅遊去哪裡玩 (難易度地點推薦)'}</span>
              </div>
              <span className="text-xs text-text-sub font-mono">
                {isEn ? 'Hourly Rate: ' : '折算時薪：'}<span className={`font-bold ${styles.themeAccentText}`}>${displayHourlyRate} NTD/hr</span>
              </span>
            </div>
            ...
            <div className="pt-3 border-t border-border-glass/50 space-y-3">
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
                <label htmlFor={exploreCountryId} className="text-xs font-bold text-text-main flex items-center gap-1.5 shrink-0">
                  ...
                  <span>{isEn ? 'Explore 152 Countries Purchasing Power:' : '探索全球 152 國時薪購買力與體感：'}</span>
                </label>
                <select
                  id={exploreCountryId}
                ...
                >
                  {numbeoInsights.evaluated
                    .slice()
                    .sort((a, b) => (isEn ? a.country.localeCompare(b.country) : a.name_zh.localeCompare(b.name_zh, 'zh-TW')))
                    .map((c) => (
                      <option key={c.country} value={c.country}>
                        {c.flag} {isEn ? c.country : c.name_zh} ({c.country}) - PPP {c.multiplierStr}x
                      </option>
                    ))}
                </select>
              </div>
              ...
              <div className="p-4 rounded-xl bg-surface-glass-btn border border-border-glass space-y-3">
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 border-b border-border-glass/40 pb-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="flex flex-wrap items-center gap-2">
                        <span className="text-base font-bold text-text-main">{isEn ? exploreTarget.country : exploreTarget.name_zh}</span>
                      ...
                      <div className="text-xs text-text-sub font-mono mt-1">
                        {isEn ? 'Matched Sphere: ' : '對應適配圈：'}<span className="font-bold text-text-main">{exploreTarget.suitabilityName}</span> 【{exploreTarget.suitabilityTag}】
                      </div>
                    </div>
                  </div>
                  <div className="flex flex-wrap items-center gap-2 text-xs font-mono shrink-0 max-sm:w-full max-sm:pt-2">
                    <span className="px-2.5 py-1 rounded-lg bg-surface-glass border border-border-glass text-text-main">
                      {isEn ? 'Local Value: ' : '折算當地購買力：'}${exploreTarget.localEffectiveRate.toLocaleString(isEn ? 'en-US' : 'zh-TW')} NTD/hr
                    </span>
                    <span className="px-2.5 py-1 rounded-lg bg-surface-glass border border-border-glass text-text-main">
                      {isEn ? 'Dining: ' : '外食：'}{exploreTarget.restSavings > 0 ? (isEn ? `Save ${exploreTarget.restSavings}%` : `省 ${exploreTarget.restSavings}%`) : exploreTarget.restSavings < 0 ? (isEn ? `${Math.abs(exploreTarget.restSavings)}% Higher` : `高 ${Math.abs(exploreTarget.restSavings)}%`) : (isEn ? 'Similar to TW' : '與台灣相近')}
                    </span>
                    <span className="px-2.5 py-1 rounded-lg bg-surface-glass border border-border-glass text-text-main">
                      {isEn ? 'Rent: ' : '租金：'}{exploreTarget.rentSavings > 0 ? (isEn ? `Save ${exploreTarget.rentSavings}%` : `省 ${exploreTarget.rentSavings}%`) : exploreTarget.rentSavings < 0 ? (isEn ? `${Math.abs(exploreTarget.rentSavings)}% Higher` : `高 ${Math.abs(exploreTarget.rentSavings)}%`) : (isEn ? 'Similar to TW' : '與台灣相近')}
                    </span>
                  </div>
                </div>
                <div className="p-3.5 rounded-lg bg-surface-glass border border-border-glass/50 text-xs text-text-sub space-y-2">
                  <div className="font-bold text-text-main text-sm flex items-center gap-1.5">
                    ...
                    <span>{isEn ? `Living experience evaluation in ${exploreTarget.country} at $${displayHourlyRate}/hr:` : `以你目前時薪 $${displayHourlyRate}/hr 在${targetName}的生活體感評估：`}</span>
                  </div>
      ...
      {nextMilestone && (
        <div>
          <h3 className="text-xs font-bold text-text-sub mb-3 flex items-center gap-1.5 uppercase tracking-wider">
            ...
            {isEn ? 'Next Rank Climbing Target' : '階級晉升天梯攻略 (Next Rank Target)'}
          </h3>
          <div className="p-4 rounded-xl bg-gradient-to-r from-[var(--theme-color)]/10 via-surface-glass to-surface-glass border border-[var(--theme-color)]/30 flex flex-wrap items-center justify-between gap-3">
            <div>
              <div className="flex items-center gap-2 mb-1">
                <span className="text-xs font-bold text-text-sub">{isEn ? 'Next Milestone Target:' : '下一個目標階級：'}</span>
                <span className="text-sm font-extrabold text-text-main">
                  【{nextMilestone.label_en && isEn ? nextMilestone.label_en : nextMilestone.label}】(PR {nextMilestone.pr})
                </span>
              </div>
              <p className="text-xs text-text-sub">
                {isEn ? (
                  <>Monthly gap to upgrade: <span className="font-bold text-text-main font-mono">+${monthlyGap.toLocaleString('en-US')} TWD</span> (or +<span className={`font-bold font-mono ${styles.themeAccentText}`}>+${hourlyGapDisplay}/hr</span>)</>
                ) : (
            ...
            <span className={`text-xs font-bold px-3 py-1.5 rounded-lg ${styles.targetBadge} font-mono`}>
              {isEn ? `Target Monthly Income ~${Math.round(nextMonthlyIncome / 1000)}k` : `晉升目標實質月收益 ~${Math.round(nextMonthlyIncome / 1000)}k`}
            </span>
          </div>
        </div>
      )}
      ...
      <div className="flex flex-wrap items-center gap-3 pt-4 border-t border-border-glass">
        ...
        >
          ...
          {isEn ? 'Calculate My Hourly Rate & PR ➔' : '試算我的時薪與 PR ➔'}
        </Link>
