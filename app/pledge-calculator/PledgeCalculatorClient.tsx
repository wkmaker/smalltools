...
公式如下：
① 借款金額上限 = 股票總市值 × 質押成數（台股常規最高成數為 60%）。
② 維持率 (%) = (股票當前總市值 ÷ 總借款金額) × 100%。例如借款 60 萬元買 100 萬股票，起始維持率即為 166.7%。`,
      },
      {
        q: '股票質押維持率低於 130% 會發生什麼事？什麼是「追繳通知」與「斷頭賣出」？',
        a: `追繳與斷頭機制如下：

① 追繳通知 (Margin Call)：當台股大跌導致質押股票總市值下滑，使整戶維持率低於 130% 時，券商或銀行會發出追繳通知，要求借款人在 2 個營業日內補繳現金或追加股票擔保品至維持率恢復至 166% 以上。
② 斷頭處分：若未在期限內補繳，券商將於第 3 個營業日開盤時，以市價強行賣出質押股票抵償借款，稱為「斷頭」。`,
      },
      {
        q: '如何精準計算股票質押的「斷頭追繳臨界股價」？',
        a: '追繳臨界股價即為維持率降至 130% 時的股票價格算式：追繳臨界股價 = (總借款金額 × 1.3) ÷ 持有總股數。例如您借款 60 萬元，持有 1,000 股，追繳臨界股價即為 (600,000 × 1.3) ÷ 1,000 = 780 元。本工具會自動試算並在地圖儀表板上醒目標示。',
      },
      {
        q: '當維持率面臨追繳警告時，有哪些補救與提升維持率的方法？',
        a: `提升維持率有三大常見策略：

① 直接償還部分本金：還款可立即降低分母（借款金額），迅速拉高維持率。
② 追加劃撥擔保股票：將名下其他優質現股劃撥質押至同一個質押帳戶，增加分子（總市值）。
③ 補繳現金保證金：依券商規定存入保證金專戶。`,
      },
      {
...
此外，特別需要注意券商「雨天收傘」風險：
① 拒絕展延或強制要求還款：質押借款期限多為 6 個月，到期時若適逢市場大跌或券商內部質押總額度用盡，券商有權拒絕申請展延，要求您一次還清本金。
② 機動調升借款利率：券商得依市場資金成本隨時調整質押利率。
...
① 維持率計算基準價：部分券商採用「前一日收盤價」，部分採用「盤中即時價」或「當日收盤價」。
② 個別股票成數上限：部分中小型股或高波動股質押成數可能低於 60%（如 50% 或 40%）。
③ 費用與手續費：撥款手續費、開戶規費與跨行轉帳費用未列入本工具利息估算。`,
      },
      {
        q: '股票質押發放的「股利與股息」歸誰所有？會被券商扣走嗎？',
        a: '股利與股息完全歸借款人所有！股票質押期間，股票所有權人仍為借款人本人。除權息時產生的現金股利會直接匯入您的交割帳戶，股票股利亦會劃撥入戶。',
      },
      {
        q: '股票質押利息如何計算？借款期限多長？',
        a: '股票質押利息按天計算（年利率 ÷ 365 × 借款天數），每半年結算一次利息。常規質押期限為 6 個月，到期前若維持率維持良好且付清利息，得申請展延 6 個月，最長可連續展延至 1.5 年至 2 年，無須強制賣股還款。',
      },
    ],
  },
  en: {
...
Formulas:
① Max Loan Amount = Total Stock Market Value × Pledge Ratio (usually up to 60% in Taiwan).
② Maintenance Ratio (%) = (Current Total Stock Value ÷ Total Loan Amount) × 100%. For instance, pledging $1M in stock to borrow $600K results in an initial maintenance ratio of 166.7%.`,
      },
      {
...
① Margin Call: If market drops reduce collateral value below 130%, brokers issue a margin call requiring additional cash or shares within 2 business days to restore maintenance above 166%.
② Forced Liquidation: Failing to meet margin calls results in brokers forcibly selling pledged shares at market open on the 3rd business day.`,
      },
      {
        q: 'How is the 130% Margin Call Threshold Stock Price calculated?',
        a: 'Margin Call Price = (Total Loan Amount × 1.3) ÷ Total Shares. For example, borrowing $600,000 against 1,000 shares yields a threshold price of ($600,000 × 1.3) ÷ 1,000 = $780. Our calculator automatically computes and displays this threshold.',
      },
      {
        q: 'How can investors increase their maintenance ratio during a market crash?',
        a: `Three common strategies:

① Repay Loan Principal: Directly reduces loan amount (denominator), rapidly elevating maintenance ratio.
② Deposit Additional Collateral Shares: Transfer other eligible stocks into the pledge account to increase total collateral value (numerator).
③ Deposit Cash Margin: Deposit cash directly into the margin account as required by brokers.`,
      },
      {
...
① Refusal to extend or demanding early repayment: Brokers retain full discretion to deny loan extensions upon 6-month term expiration, especially during market turmoil or when total lending quotas are reached.
② Floating interest rate hikes: Brokers may adjust pledge interest rates based on market capital costs.
...
① Reference Price for Maintenance Ratio: Some brokers use previous day closing prices, while others use real-time intraday or daily closing prices.
② Pledge Ratio Caps: High-volatility or small-cap stocks may have lower pledge ratios (e.g. 40%–50% instead of 60%).
③ Fees & Charges: Origination fees or wire transfer costs are excluded from calculations.`,
      },
      {
        q: 'Who receives dividends and stock distributions during stock pledging?',
        a: 'Dividends and stock splits belong 100% to the borrower! The borrower retains underlying stock ownership throughout the pledge term. Cash dividends are deposited into your settlement account.',
      },
      {
        q: 'How is pledge loan interest calculated, and what is the maximum loan duration?',
        a: 'Interest is calculated daily (Annual Rate ÷ 365 × Days) and settled semi-annually. Standard terms are 6 months, expandable up to 1.5–2 years upon request provided maintenance ratios remain healthy.',
      },
    ],
  },
};
