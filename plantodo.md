...
## 狀態：✅ 已完成（2026-09-19，見 CHANGELOG [1.16.0]）

第五節邊緣案例 5.3 已由使用者確認採用「只存 `activeTab`，不存 `searchQuery`」方案並照此實作。
本文件保留作為施工紀錄，程式碼現況以 `app/HomeClient.tsx`／`app/translations.ts` 為準。
